...
class Api extends BaseTreeApi<Disk.StoreFile, number> {
  ...
  /** 打标签 */
  addTags = (fileIds: number[], tagIds: number[]): Promise<Fa.Ret<boolean>> => this.post('addTags', { fileIds, tagIds });

  /** 获取实体List */
  queryFile = (params: Fa.BasePageQuery<Disk.StoreFileQuery>): Promise<Fa.Ret<Disk.StoreFile[]>> => this.post('queryFile', params);

  /** 检索文件分页 */
  queryFilePage = (params: Fa.BasePageQuery<Disk.StoreFileQuery>): Promise<Fa.Ret<Fa.Page<Disk.StoreFile>>> => this.post('queryFilePage', params);

  /** 检索回收站文件分页 */
  queryTrashFilePage = (params: Fa.BasePageQuery<Disk.StoreFileQuery>): Promise<Fa.Ret<Fa.Page<Disk.StoreFile>>> => this.post('queryTrashFilePage', params);
